...
PRODUCTS = {
    "oxide": {
        "list_char": "🧊",
        "list_emoji": EMOJI_OXIDE,
        "name": "oxide",
        "price": "уточняется",  # заполните реальную цену
        "product_char": "🧊",
        "product_emoji": EMOJI_OXIDE,
    },
    "cry4me": {
        "list_char": "😢",
...
async def receive_receipt(message: Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Принять", callback_data=f"approve_{order['id']}"),
