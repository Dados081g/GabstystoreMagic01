...
async def fulfill_order(bot: Bot, order_id: int) -> bool:
    """Пытается выдать ключ по заказу. Возвращает True, если ключ выдан."""
    conn = db()
...
async def admin_add_keys_start(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        ...
        "Если категория одна — можно присылать просто ключи:\n"
        "AAAA-BBBB-CCCC\nDDDD-EEEE-FFFF\n\n"
        "Если категорий несколько — указывайте формат категория|ключ:\n"
...
async def admin_add_keys_save(message: Message, state: FSMContext):
    ...
    default_category = next(iter(PRODUCTS)) if len(PRODUCTS) == 1 else None
    ...
    for line in lines:
        if "|" in line:
            ...
            category, key_value = default_category, line
        else:
            skipped += 1
            continue
    ...
    if skipped:
        result += (
            f"\nПропущено строк: {skipped} — у вас несколько категорий, "
            f"используйте формат категория|ключ"
        )

    # Авто-выдача: если кого-то уже одобрили, но раньше не хватало ключей — выдаём сейчас
    conn = db()
    ...
    for row in waiting:
        if await fulfill_order(message.bot, row["id"]):
            auto_issued += 1
    if auto_issued:
        result += f"\n🔑 Автоматически выдано ключей по ожидавшим заказам: {auto_issued}"
