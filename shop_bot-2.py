...
EMOJI_GOOGLE = "5290018666232973726"   # 🔑 в категории Google account

# ID премиум-стикера для категории "Android • БЕЗ РУТ"
STICKER_ANDROID_NOROOT = "5454386656628991407"
...
MAIN_KB = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="📋 Соглашение"), KeyboardButton(text="❓ Помощь")],
    ],
...
#
# Два вида категорий:
#   type="product" — категория с одним или несколькими вариантами покупки
#                     (у каждого варианта своя цена и свой пул ключей в БД).
#   type="info"     — информационная категория без покупки (просто текст + стикер).
#
# Порядок в словаре = порядок в списке "Выберите категорию".
...
CATEGORIES = {
    ...
    "google_account": {
        "type": "product",
        "list_char": "🔑",
        "list_emoji": EMOJI_GOOGLE,
        "product_char": "🔑",
        "product_emoji": EMOJI_GOOGLE,
        "variants": {
            # ключ варианта "main" -> composite-ключ в БД = "google_account_main"
            "main": {
                "label": "Google аккаунт",
                "price_key": "price_google_account",
                "default_price": "20 руб",  # цену можно поменять в /admin → 💰 Цены
            },
        },
    },
    "android_noroot": {
        ...
        "list_char": "🤖",
        "list_emoji": None,
        "sticker": STICKER_ANDROID_NOROOT,
        "text": (
            "Android • БЕЗ РУТ\n"
            "━━━━━━━━━━━━━━\n"
            "⭐ Читы которые не требуют ROOT прав\n"
            "━━━━━━━━━━━━━━\n"
            "⭐ Если вы не знаете что такое ROOT права, то скорее всего у вас их нет "
            "и данная категория для вас подходит.\n"
            "Ознакомьтесь с утилитами, у каждого разный функционал и внешний вид"
        ),
    },
}
...
async def show_categories(message: Message):
    ...
    for key, c in CATEGORIES.items():
        lines.append(f"{c['list_char']} {key}")
    text = "\n".join(lines)

    entities = [emoji_entity(text, "🛍", EMOJI_CATEGORY)]
    for key, c in CATEGORIES.items():
        if c.get("list_emoji"):
            entities.append(emoji_entity(text, c["list_char"], c["list_emoji"]))

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=key, callback_data=f"cat_{key}")] for key in CATEGORIES
        ]
    )
...
async def agreement_and_privacy(message: Message):
    ...
    await message.answer("Пользовательское соглашение и политика конфиденциальности:", reply_markup=kb)
...
async def show_product(callback: CallbackQuery):
    ...
    # Информационная категория (например "Android • БЕЗ РУТ") — без покупки.
    if c["type"] == "info":
        if c.get("sticker"):
            await callback.message.answer_sticker(c["sticker"])
        await callback.message.answer(c["text"])
        await callback.answer()
        return

    variants = c["variants"]

    # Один вариант — сразу показываем карточку товара с кнопкой "Купить".
    if len(variants) == 1:
        var_key = next(iter(variants))
        var = variants[var_key]
        price = get_variant_price(key, var_key)
        comp = composite_key(key, var_key)
        text = f"{c['product_char']} {var['label']} — {price}"
        entities = []
        if c.get("product_emoji"):
            entities.append(emoji_entity(text, c["product_char"], c["product_emoji"]))
        kb = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="🛒 Купить", callback_data=f"buy_{comp}")]]
        )
        await callback.message.edit_text(text, entities=entities, reply_markup=kb)
        await callback.answer()
        return

    # Несколько вариантов (например cry4me 1D / 7D) — список кнопок с ценами.
    lines = [f"{c['product_char']} Выберите вариант:"]
    kb_rows = []
    for var_key, var in variants.items():
        price = get_variant_price(key, var_key)
        comp = composite_key(key, var_key)
        kb_rows.append([InlineKeyboardButton(text=f"{var['label']} — {price}", callback_data=f"buy_{comp}")])
    text = "\n".join(lines)
    entities = []
    if c.get("product_emoji"):
        entities.append(emoji_entity(text, c["product_char"], c["product_emoji"]))
...
async def admin_add_keys_start(callback: CallbackQuery, state: FSMContext):
    categories_hint = "\n".join(
        f"{composite_key(ck, vk)}" for ck, c in CATEGORIES.items() if c["type"] == "product" for vk in c["variants"]
    )
    await callback.message.answer(
        "Пришлите ключи/аккаунты, каждый с новой строки, в формате категория|значение:\n\n"
        f"{categories_hint}|ЗНАЧЕНИЕ\n\n"
        "Для google_account значение указывайте как логин:пароль, например:\n"
        "google_account_main|example@gmail.com:parol123\n\n"
        "Для остальных категорий — просто ключ:\n"
        "cry4me_1d|AAAA-BBBB-CCCC"
    )
...
async def admin_add_keys_save(message: Message, state: FSMContext):
    ...
    for line in lines:
        ...
        category, key_value = category.strip(), key_value.strip()

        if not category or not key_value:
            skipped += 1
            continue
    ...
    if skipped:
        result += f"\nПропущено строк: {skipped} — используйте формат категория|значение"
